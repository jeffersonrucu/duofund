    ...
    <div class="rise" style="animation-delay:.1s"
        ...
        <form method="POST" action="{{ route('register.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <div class="relative">
                    <x-lucide-user class="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                ...
                @else
                    <p class="mt-1.5 flex items-center gap-1 text-xs transition-colors"
                       :class="pw.length >= 8 ? 'text-green-600' : 'text-gray-400'">
                        <x-lucide-check-circle-2 x-show="pw.length >= 8" class="h-3.5 w-3.5" />
                        <x-lucide-info x-show="pw.length < 8" class="h-3.5 w-3.5" />
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <button type="button" @click="showConfirm = !showConfirm" tabindex="-1"
                            class="absolute right-3 top-1/2 -translate-y-1/2 rounded-md p-1 text-gray-400 transition hover:text-gray-600">
                        <x-lucide-eye x-show="!showConfirm" class="h-4 w-4" />
                        <x-lucide-eye-off x-show="showConfirm" x-cloak class="h-4 w-4" />
                    </button>
                </div>
                <p x-cloak x-show="pwc.length > 0" class="mt-1.5 flex items-center gap-1 text-xs font-medium transition-colors"
                   :class="pw === pwc ? 'text-green-600' : 'text-amber-600'">
                    <x-lucide-check-circle-2 x-show="pw === pwc" class="h-3.5 w-3.5" />
                    <x-lucide-alert-circle x-show="pw !== pwc" class="h-3.5 w-3.5" />
